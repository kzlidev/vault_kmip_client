...
class KMIPProxy(object):
    ...
    def open(self):
        ...
        self.logger.debug(
            "KMIPProxy cert_reqs: {0} (CERT_REQUIRED: {1})".format(
                self.cert_reqs, custom_ssl.CERT_REQUIRED))
        self.logger.debug(
            "KMIPProxy ssl_version: {0} (PROTOCOL_SSLv23: {1})".format(
                self.ssl_version, custom_ssl.PROTOCOL_SSLv23))
    ...
    def _create_socket(self, sock):
        self.socket = custom_ssl.wrap_socket(
            sock,
    ...
    def _set_variables(self, host, port, keyfile, certfile,
        ...
        self.cert_reqs = getattr(custom_ssl, conf.get_valid_value(
            cert_reqs, self.config, 'cert_reqs', 'CERT_REQUIRED'))

        self.ssl_version = getattr(custom_ssl, conf.get_valid_value(
